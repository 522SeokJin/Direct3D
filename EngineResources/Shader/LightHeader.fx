...
float4 CalculateDirectionDiffuseLight(float4 _vViewNormal, LightData _Light)
{
    ...
    return DiffuseLight * _Light.DiffuseLightColor * 
    _Light.LightPower.x * _Light.LightPower.w;

}

float4 CalculateDirectionSpacularLight(float4 _vViewPosition, float4 _vViewNormal, 
LightData _Light)
{
    ...
    float3 Reflection = normalize((2.0f * _vViewNormal.xyz *
    dot(_Light.ViewNegLightDir.xyz, _vViewNormal.xyz)) - _Light.ViewNegLightDir.xyz); // N
    ...
    return SpacularLight * _Light.SpacularLightColor * 
    _Light.LightPower.y * _Light.LightPower.w;
}
